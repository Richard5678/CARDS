...
class BaseRewardSampling:
    ...
    def from_text_to_token(self, text):
        ...
        tokens, mask = out.input_ids.to(self.LLM.device), out.attention_mask.to(
            self.LLM.device
        )
        return tokens, mask
    ...
    def from_token_to_onehot_token(self, token):
        onehot_tokens = F.one_hot(token, num_classes=self.tokenizer.vocab_size).to(
            token.device
        )
    ...
    def from_onehot_token_to_embedding(self, onehot_token, model):
        assert onehot_token.dtype == torch.float32, (
            "onehot_tokens must be of type torch.float32, not %s!" % onehot_token.dtype
        )
    ...
    def from_logit_to_token(
        self, logit, discard_token=None, top_k: int = 1, temperature: float = 1.0
    ):
        if discard_token is not None:
            batch_idx = torch.arange(
                logit.shape[0], dtype=discard_token.dtype, device=discard_token.device
            )
        ...
        if top_k == 1:
            ...
            selected_idx = torch.multinomial(
                F.softmax(val / temperature, dim=-1), num_samples=1
            )
    ...
    def from_token_to_full_logit(self, token, mask=None):
        prepared_inputs = self.LLM.prepare_inputs_for_generation(
            input_ids=token, attention_mask=mask, use_cache=False
        )
        return self.LLM(**prepared_inputs).logits
